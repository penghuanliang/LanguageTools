...
from simple_use import XLSXToXML
...
class UiXLSXToXML:
    ...
    def select_dir(self):
        out_options = {
            'initialfile': 'C:/Users/Administrator/Desktop/LanguagesFile.xlsx',
            'title': '选择一个xls 或 xlsx 文件'
        }
        _out = askopenfilename(**out_options)
        self.e1.set(_out)
    ...
    def run(self):
        ...
        if e1_path.exists() is False or e2_path.exists() is False or self.e1.get() == '' or self.e2.get() == '':
            self.show_msg("文件不存在！请检查文件或目录是否存在！")
            return

        _ui = XLSXToXML()
        _ui.run(self.e1.get(), self.e2.get())
    ...
    def __init__(self, parent):
        ...
        Label(master=frame, text='选择一个.xlsx 或 .xls文件', font=('Arial', 14)).grid(row=0, column=0, sticky=W, padx=15,
                                                                                pady=15)
        Label(master=frame, text='选择一个res目录', font=('Arial', 14)).grid(row=2, column=0, sticky=W, padx=15)
        ...
        Button(master=frame, text='打开', command=self.select_dir, cnf=self.btn_style).grid(row=1, column=1,
                                                                                          sticky=W,
                                                                                          padx=10,
                                                                                          pady=10)
        Button(master=frame, text='打开', command=self.select_res_file, cnf=self.btn_style).grid(row=3, column=1,
                                                                                               sticky=W,
                                                                                               padx=10, pady=10)
